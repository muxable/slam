...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y git \
    ...
    libssl-dev libboost-dev libboost-filesystem-dev libboost-serialization-dev
...
RUN cd ORB_SLAM3 && mkdir build && cd build && cmake .. -DCMAKE_BUILD_TYPE=Release && make -j$(nproc)